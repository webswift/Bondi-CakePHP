 ...
 */
	...
	public function getUserListInGroup()
	{
		$groupID = $_POST['groupID'];
		$this->Session->write('workingGroupID', $groupID);

		$options = array('conditions' => array('group_id' => $groupID));
		$users = $this->GroupsUser->find('all', $options);

		$values = array();

		foreach ($users as $user){
			$values[] = array('id' => $user['GroupsUser']['id'],
				'userID' => $user['User']['id'], 'userName' => $user['User']['username'],
				'groupID' => $user['Group']['id'], 'groupName' => $user['Group']['name']);
		}
